...
const AppointmentForm = ({ theme }) => {
  ...
  const { data: appointments = [], isLoading: isFetching, refetch } =
    useGetAppointmentsQuery();

  const {
    data: closedSlotsData = [],
    isLoading: isFetchingClosed,
    error: closedSlotsError,
  } = useGetClosedSlotsQuery();
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (isDateClosed(date) || isTimeSlotClosed(date, timeSlot)) {
        toast.error("This slot or date is temporarily closed.");
        return;
      }
  ...
  const isTimeSlotBooked = (selectedDate, slot) =>
    appointments.some(
      (appointment) =>
        format(new Date(appointment.date), "MM-dd-yyyy") === selectedDate &&
        appointment.timeSlot === slot
    );

  const isDateClosed = (selectedDate) =>
    closedSlots.some(
      (entry) =>
        format(new Date(entry.date), "MM-dd-yyyy") === selectedDate &&
        entry.timeSlots.length === 0
    );

  const isTimeSlotClosed = (selectedDate, slotValue) =>
    closedSlots.some(
      (entry) =>
        format(new Date(entry.date), "MM-dd-yyyy") === selectedDate &&
        entry.timeSlots.includes(slotValue)
    );
  ...
  const isDateFullyBookedOrClosed = (selectedDate) =>
    isDateClosed(selectedDate) || isDateFullyBooked(selectedDate);
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              ...
              <Select
                ...
                onChange={(e) => setTimeSlot(e.target.value)}
                fullWidth
                disabled={!date || isDateClosed(date)}
              >
                {timeSlots.map((option) => {
                  const booked = isTimeSlotBooked(date, option.value);
                  const closed = isTimeSlotClosed(date, option.value);
            ...
            <Grid item xs={12}>
              <FormControl fullWidth>
                <InputLabel htmlFor="notes">Notes</InputLabel>
                <Input id="notes" {...formik.getFieldProps("notes")} />
                {formik.touched.notes && formik.errors.notes && (
                  <FormHelperText error>
                    {formik.errors.notes}
                  </FormHelperText>
